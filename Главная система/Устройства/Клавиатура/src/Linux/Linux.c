...
static Byte key_map[32];
static Byte key_table[] = {
...
{
	Byte           new_key_code;
	N_32           key_code_position;

	static X11     lib     = X11.INSTANCE;
	static Display display = lib.XOpenDisplay(0);

	XQueryKeymap(display, key_map);
	real_key_code = key_table[key_code];
	key_code_position = 1<<(real_key_code % 16 - 1);

	if(pos & keymap[real_key_code/16])
		return 1;
